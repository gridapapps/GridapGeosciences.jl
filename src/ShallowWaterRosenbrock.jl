...
function shallow_water_rosenbrock_time_stepper(model, order, degree,
  ...
  function run_simulation(pvd=nothing)
    ...
    if (write_diagnostics && write_diagnostics_freq>0 && mod(istep, write_diagnostics_freq) == 0)
      ...
      compute_potential_vorticity!(q1, H1h, H1hchol, dΩ, R, S, hn, un, f, get_normal_vector(Ω))
      dump_diagnostics_shallow_water!(h_tmp, w_tmp,
                                      model, dΩ, dω, S, L2MM, H1MM,
                                      hn, un, wn, q1, ϕ, F, g, istep, dt,
                                      diagnostics_file,
...
function shallow_water_rosenbrock_time_step_apvm!(
...
function shallow_water_rosenbrock_time_stepper_apvm(model, order, degree,
                        h₀, u₀, f₀, g, H₀,
                        λ, dt, τ, N; leap_frog=false,
                        write_diagnostics=true,
                        write_diagnostics_freq=1,
                        ...
                        write_solution=false,
                        write_solution_freq=N/10,
                        output_dir="nswe_ncells_$(num_cells(model))_order_$(order)_rosenbrock_apvm")
  ...
  function run_simulation(pvd=nothing)
    ...
    # first step, no leap frog
    istep = 1
    shallow_water_rosenbrock_time_step_apvm!(yn, ϕ, F, q1, q2, duh1, duh2, H1h, H1hchol, y_wrk,
  ...
  if (write_solution)
    pvdfile=joinpath(output_dir,"nswe_ncells_$(num_cells(model))_order_$(order)_rosenbrock_apvm")
    paraview_collection(run_simulation,pvdfile)
  else
    run_simulation()
  end
