...
function compute_buoyancy_ibp!(e,dΩ,dΓ,n,R,S,H1MM,H1chol,E,h,u,τ)
  #b(s) = ∫(s*E + τ*E*(∇⋅(s*u)))dΩ - ∫(τ*mean(E)*jump((s*u)⋅n))dΓ
  b(s) = ∫(s*E)dΩ
  Gridap.FESpaces.assemble_vector!(b, get_free_dof_values(e), S)
  ldiv!(H1chol, get_free_dof_values(e))
end

function update_velocity_tswe_ibp!(u1,dΩ,dω,dΓ,V,RTMMchol,u2,q,e,F,ϕ,T,n,dt)
  b(v) = ∫(v⋅u2 - dt*(q)*(v⋅⟂(F,n)) + dt*(∇⋅(e*v))*T)dΩ - ∫(dt*mean(T)*jump((e*v)⋅n))dΓ + ∫(dt*DIV(v)*ϕ)dω
  Gridap.FESpaces.assemble_vector!(b, get_free_dof_values(u1), V)
  ldiv!(RTMMchol, get_free_dof_values(u1))
end

function update_buoyancy_ibp!(E1,dΩ,dω,dΓ,Q,L2MMchol,E2,e,F,n,dt)
  b(q) = ∫(q*E2 - dt*q*(∇⋅(e*F)))dΩ + ∫(dt*mean(q)*jump((e*F)⋅n))dΓ
  Gridap.FESpaces.assemble_vector!(b, get_free_dof_values(E1), Q)
  ldiv!(L2MMchol, get_free_dof_values(E1))
end

function thermal_shallow_water_ibp_explicit_time_step!(
     h₂, u₂, E₂, hₚ, uₚ, Eₚ, ϕ, F, q₁, q₂, e₁, e₂, H1h, H1hchol, L2h, L2hchol, dT, # in/out args
     model, dΩ, dω, dΓ, V, P, Q, R, S, f, h₁, u₁, E₁, hₘ, uₘ, Eₘ, e₁up, e₂up,      # in args
  ...
  compute_buoyancy_ibp!(e₁,dΩ,dΓ,n,R,S,H1h,H1hchol,E₁,h₁,u₁,τₑ)
  # 1.4: solve for the provisional velocity
  update_velocity_tswe_ibp!(uₚ,dΩ,dω,dΓ,V,RTMMchol,uₘ,q₁-τ*u₁⋅∇(q₁),e₁,F,ϕ,0.5*h₁,n,dt1)
  ...
  update_buoyancy_ibp!(Eₚ,dΩ,dω,dΓ,Q,L2MMchol,Eₘ,e₁,F,n,dt1)
  ...
  compute_buoyancy_ibp!(e₂,dΩ,dΓ,n,R,S,H1h,H1hchol,Eₚ,hₚ,uₚ,τₑ)
  # 2.4: solve for the final velocity
  update_velocity_tswe_ibp!(u₂,dΩ,dω,dΓ,V,RTMMchol,u₁,0.5*(q₁-τ*u₁⋅∇(q₁)+q₂-τ*uₚ⋅∇(q₂)),0.5*(e₁+e₂),F,ϕ,0.25*(h₁+hₚ),n,dt)
  # 2.5: solve for the final depth
  compute_depth!(h₂,dΩ,dω,Q,L2MMchol,h₁,F,dt)
  # 2.6: solve for the buoyancy weighted mass flux
  update_buoyancy_ibp!(E₂,dΩ,dω,dΓ,Q,L2MMchol,E₁,0.5*(e₁+e₂),F,n,dt)
end

function thermal_shallow_water_ibp_explicit_time_stepper(model, order, degree,
                        h₀, u₀, E₀, f₀,
                        dt, τ, τₑ, N;
  ...
  function run_simulation(pvd=nothing)
    ...
    thermal_shallow_water_ibp_explicit_time_step!(hn, un, En, hp, up, Ep, ϕ, F, q1, q2, e1, e2, H1h, H1hchol, L2h, L2hchol, eF,
                                              model, dΩ, dω, dΓ, V, P, Q, R, S, f, hm1, um1, Em1, hm2, um2, Em2, e1up, e2up,
