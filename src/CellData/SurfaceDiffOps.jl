...
#### Contravariant components of 2D vector laplacian, required for Stokes
...
curly_curl(u, m) = x -> 1/sqrtg(m)(x)*perp(gradient(skew_surfdiv(contra_v(u),m))(x))
...
vec_laps_2D(u,m) = x ->  contra_grads_divs(u,m)(x) + curly_curl(u,m)(x)
